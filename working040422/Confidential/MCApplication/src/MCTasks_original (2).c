...
uint16_t FOC_CurrController(uint8_t bMotor)
{
  ...
  obs_ele_pll_angle = ((int16_t)UI_GetReg(oDAC,MC_PROTOCOL_REG_OBS_EL_ANGLE));

  /* Uncomment to use sensored data for speed regulation*/
  speedangle = myangle;

  /* Uncomment to use sensorless data for speed regulation*/
  //speedangle = obs_ele_pll_angle;
...
//#define GETANGLE 0 // - - JDO change to 1 to lock motor to 0 for timing get angle ------
  ...
  {
    //  ** NORMAL OPERATION ** //
    myangle = myangle + timing_adj;  // angle alignment adjustment
    ...
    if(current_motor_mode == ENCODER)
    {
        hElAngledpp = myangle;
    }
    else if(current_motor_mode == SENSORLESS)
    {
        hElAngledpp = obs_ele_pll_angle;
    }

    /* Uncomment to output to DAC channel 1 */
//    DAC->DHR12L1 = myangle;
//    DAC->SWTRIGR=1;

    /* Uncomment to output to DAC channel 2 */
//    DAC->DHR12L2 = obs_ele_pll_angle;
//    DAC->SWTRIGR=3;
}
...
// dcnq17 substitute MA302 angle data
