...
static void ENC_Init(CSPD this)
{
  ...
  uint32_t wRCC_AHBPeriph_DMAx = RCC_AHBPeriph_DMA1;
  ...
  {
    wTimerStatusRegisterAddress = (uint32_t)TIM5_SR_Address;
    wRCC_AHBPeriph_DMAx = RCC_AHBPeriph_DMA2;
    DMAPeriph_Channel = DMA2_Channel1;
  }
  else
  {
    ;
  ...
  /* DMAx clock enable */
  RCC_AHBPeriphClockCmd(wRCC_AHBPeriph_DMAx, ENABLE);  
  ...
  if(pDParams_str->wTIMxRemapping != GPIO_NoRemap_TIMx)
  {   
    GPIO_PinRemapConfig(pDParams_str->wTIMxRemapping, ENABLE);
    /* Disable JTAG pins if required */
    if ((TIMx == TIM2) && ((pDParams_str->wTIMxRemapping == GPIO_PartialRemap1_TIM2) || (pDParams_str->wTIMxRemapping == GPIO_FullRemap_TIM2)))
    {
    ...
    if ((TIMx == TIM3) && (pDParams_str->wTIMxRemapping == GPIO_PartialRemap_TIM3))
    {
      /* Only PA15 and PB3 are released JTAG-DP Disabled and SW-DP Enabled */
      GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable,ENABLE);
      /* To disable all JTAG/SWD pins Full SWJ Disabled (JTAG-DP + SW-DP) uncomment line below */
  ...
  TIM_TimeBaseStructure.TIM_Period = pDParams_str->hPulseNumber - 1u;  
...
static void ENC_IRQHandler(void *this, unsigned char flag)
{
  ...
  {
    hNewCapture = *(pDVars_str->hTIMxCCRAddress);
