/**
  ******************************************************************************
  * @file    ENC_F30x_SpeednPosFdbkClass.c
...
#include "ENC_F30X_SpeednPosFdbkPrivate.h"
...
static void ENC_Init(CSPD this)
{
  ...
  uint16_t hTIM_Channel_x = TIM_Channel_4;  
  uint16_t hTIM_IT_CCx = TIM_IT_CC4;
  uint16_t hTIM_DMA_CCx = TIM_DMA_CC4;  
  
  pDParams_t pDParams_str = DCLASS_PARAM;
  pDVars_t pDVars_str = DCLASS_VARS;
  TIM_TypeDef* TIMx = pDParams_str->TIMx;
  DMA_Channel_TypeDef *DMAPeriph_Channel = 0;
  ...
  pDVars_str->wTIMxCCRAddress = &(TIMx->CCR4);
  pDVars_str->hTIM_FLAG_CCx = (uint16_t)(TIM_FLAG_CC4);
  pDVars_str->hTIM_EventSource_CCx = TIM_EventSource_CC4;
  
  if (TIMx == TIM2)
  {
    wTimerStatusRegisterAddress = (uint32_t)TIM2_SR_Address;
    DMAPeriph_Channel = DMA1_Channel7;
    bChA_AF = 1u;
    bChB_AF = 1u;
    if (pDParams_str->hAPort == GPIOD)
    {
      bChA_AF = 2u;
    }
    if (pDParams_str->hBPort == GPIOD)
    {
      bChB_AF = 2u;
    }
  }
  else if (TIMx == TIM3)
  {
    ...
    DMAPeriph_Channel = DMA1_Channel3;
    bChA_AF = 2u;
    bChB_AF = 2u;    
  }
  else if (TIMx == TIM4)
  {
    hTIM_Channel_x = TIM_Channel_3;
    hTIM_IT_CCx = TIM_IT_CC3;
    hTIM_DMA_CCx = TIM_DMA_CC3;
    pDVars_str->wTIMxCCRAddress = &(TIMx->CCR3);
    pDVars_str->hTIM_FLAG_CCx = (uint16_t)(TIM_FLAG_CC3);    
    pDVars_str->hTIM_EventSource_CCx = TIM_EventSource_CC3;    
    
    wTimerStatusRegisterAddress = (uint32_t)TIM4_SR_Address;
    DMAPeriph_Channel = DMA1_Channel5;
    bChA_AF = 2u;
    bChB_AF = 2u;
    if (pDParams_str->hAPort == GPIOA)
    {
      bChA_AF = 10u;
    }
    if (pDParams_str->hBPort == GPIOA)
    {
      bChB_AF = 10u;
    }    
  }
  ...
  /****** TIMx alternate function remapping ******/  
  GPIO_PinAFConfig(pDParams_str->hAPort, F30X_GPIOPin2Source(pDParams_str->hAPin), bChA_AF);
  GPIO_PinAFConfig(pDParams_str->hBPort, F30X_GPIOPin2Source(pDParams_str->hBPin), bChB_AF);
  ...
  DMA_Init(DMAPeriph_Channel, &DMA_InitStructure);
  ...
  DMA_Cmd(DMAPeriph_Channel, ENABLE);
...
static void ENC_IRQHandler(void *this, unsigned char flag)
{
  ...
  uint16_t hNewCapture;
  uint32_t wNewCapture;
  uint32_t wNewCounter;
  
  /*TIMx Capture IRQ*/
  if ((TIMx->SR & pDVars_str->hTIM_FLAG_CCx) == pDVars_str->hTIM_FLAG_CCx) 
  {
    wNewCapture = *(pDVars_str->wTIMxCCRAddress);
    hNewCapture = (uint16_t)wNewCapture;
    ...
    {
      wNewCounter = TIMx->CNT;
      hNewCapture = (uint16_t)wNewCounter;
    ...
    {/* encoder timer down-counting*/
      pDVars_str->wDeltaCapturesBuffer[pDVars_str->bDeltaCapturesIndex]= 
        (int32_t)(hNewCapture) - (int32_t)(pDVars_str->hPreviousCapture) -
    ...
    pDVars_str->hPreviousCapture = hNewCapture;
  }
