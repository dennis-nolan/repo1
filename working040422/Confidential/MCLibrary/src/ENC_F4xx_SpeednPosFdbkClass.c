/**
  ******************************************************************************
  * @file    ENC_F4xx_SpeednPosFdbkClass.c
  * @author  STMicroelectronics - System Lab - MC Team
  * @version 4.2.0
  * @date    20-Aug-2015 18:06
  * @brief   This file contains private implementation of ENCODER class for F4xx
...
#include "ENC_F4xx_SpeednPosFdbkPrivate.h"
...
static void ENC_Init(CSPD this)
{
  ...
  uint32_t DMAPeriph_Channel = 0u;
  DMA_Stream_TypeDef*  DMA1_Streamx = 0;
  ...
  if (TIMx == TIM2)
  {
    ...
    DMAPeriph_Channel = DMA_Channel_3;
    DMA1_Streamx = DMA1_Stream6;
    bGPIOAF = GPIO_AF_TIM2;
  }
  else if (TIMx == TIM3)
  {
    wTimerStatusRegisterAddress = (uint32_t)TIM3_SR_Address;
    DMAPeriph_Channel = DMA_Channel_5;
    DMA1_Streamx = DMA1_Stream5;
  ...
  else if (TIMx == TIM4)
  {
    ...
    DMAPeriph_Channel = DMA_Channel_2;
    DMA1_Streamx = DMA1_Stream7;
    bGPIOAF = GPIO_AF_TIM4;    
  }
  else if (TIMx == TIM5)
  {
    wTimerStatusRegisterAddress = (uint32_t)TIM5_SR_Address;
    DMAPeriph_Channel = DMA_Channel_6;
    DMA1_Streamx = DMA1_Stream1;
  ...
  GPIO_PinAFConfig(pDParams_str->hAPort, F4XX_GPIOPin2Source(pDParams_str->hAPin), bGPIOAF);
  GPIO_PinAFConfig(pDParams_str->hBPort, F4XX_GPIOPin2Source(pDParams_str->hBPin), bGPIOAF);
  ...
  DMA_DeInit(DMA1_Streamx);
  DMA_InitStructure.DMA_Channel = DMAPeriph_Channel;  
  DMA_InitStructure.DMA_PeripheralBaseAddr = wTimerStatusRegisterAddress;
  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)(&(pDVars_str->wTimerStatusRegisterCopy));
  ...
  DMA_Init(DMA1_Streamx, &DMA_InitStructure);
