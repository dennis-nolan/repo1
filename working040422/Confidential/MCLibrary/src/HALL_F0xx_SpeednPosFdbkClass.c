...
#include "HALL_F0xx_SpeednPosFdbkPrivate.h"
...
static void HALL_Init(CSPD this)
{
  ...
  GPIO_PinAFConfig(pDParams_str->hH1Port, F0XX_GPIOPin2Source(pDParams_str->hH1Pin), bGPIOAF);
  GPIO_PinAFConfig(pDParams_str->hH2Port, F0XX_GPIOPin2Source(pDParams_str->hH2Pin), bGPIOAF);
  GPIO_PinAFConfig(pDParams_str->hH3Port, F0XX_GPIOPin2Source(pDParams_str->hH3Pin), bGPIOAF);
  ...
  */ 
  TIM_PrescalerConfig(TIMx, (uint16_t) pDVars_str->hHALLMaxRatio, 
  ...
  NVIC_InitHALLStructure.NVIC_IRQChannelPriority = 
  ...
  else  
  {
    ...
    if(PeriodMeasAux.bDirection != HALL_ERROR)
    {
      ...
      else
      {
        ...
        {  
          /*Avoid uint32_t DIV Overflow*/
          if ( PeriodMeasAux.wPeriod > (uint32_t)pDVars_str->wSpeedOverflow )
          {
            pVars_str->hElSpeedDpp = (int16_t)((uint16_t) (pDVars_str->wPseudoFreqConv /
  ...
  {    
    ...
    if (pDParams_str->bSensorPlacement == DEGREES_120) 
    {
      ...
      case STATE_5:
        ...
        if(pDVars_str->bSpeed<0)
        {
          pDVars_str->hMeasuredElAngle = (int16_t)(pDParams_str->hPhaseShift+S16_60_PHASE_SHIFT);
        }
      ...
      case STATE_1:
        ...
        if(pDVars_str->bSpeed<0)
        {
          pDVars_str->hMeasuredElAngle = (int16_t)(pDParams_str->hPhaseShift+S16_120_PHASE_SHIFT);
        }
        else if(pDVars_str->bSpeed!= HALL_ERROR)
        {
      ...
      case STATE_4: 
        ...
        if(pDVars_str->bSpeed<0)
        {
          pDVars_str->hMeasuredElAngle =(int16_t)(pDParams_str->hPhaseShift);  
        }
        else if(pDVars_str->bSpeed!= HALL_ERROR)
        {
          pDVars_str->hMeasuredElAngle =(int16_t)(pDParams_str->hPhaseShift - S16_60_PHASE_SHIFT); 
...
static int16_t HALL_CalcAvrgElSpeedDpp(CSPD this)
{
  ...
  if (pDVars_str->bNewSpeedAcquisition == 1)
  {
    ...
    if (pDVars_str->bBufferFilled < pDParams_str->bSpeedBufferSize)
    {
      ...
      TIMx->DIER &= (uint16_t)~TIM_IT_CC1; /* NB:Std libray not used for perf issues*/
...
static void HALL_Init_Electrical_Angle(CSPD this)
{
  ...
  if (pDParams_str->bSensorPlacement == DEGREES_120) 
  {
    ...
    {
    case STATE_5:
      pVars_str->hElAngle = (int16_t)(pDParams_str->hPhaseShift+S16_60_PHASE_SHIFT/2);
      break;
    case STATE_1:
      pVars_str->hElAngle =(int16_t)(pDParams_str->hPhaseShift+S16_60_PHASE_SHIFT+
                               S16_60_PHASE_SHIFT/2);
      break;
    case STATE_3:
      pVars_str->hElAngle =(int16_t)(pDParams_str->hPhaseShift+S16_120_PHASE_SHIFT+
                               S16_60_PHASE_SHIFT/2);      
      break;
    case STATE_2:
      pVars_str->hElAngle =(int16_t)(pDParams_str->hPhaseShift-S16_120_PHASE_SHIFT-
