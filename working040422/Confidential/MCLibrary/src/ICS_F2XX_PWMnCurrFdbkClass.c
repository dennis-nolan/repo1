/**
  ******************************************************************************
  * @file    ICS_F2XX_PWMnCurrFdbkClass.c
...
#include "ICS_F2XX_PWMnCurrFdbkPrivate.h"
...
#define DCLASS_PARAMS ((_DCIF2XX_PWMC)(((_CPWMC) this)->DerivedClass))->pDParams_str
#define DCLASS_VARS  ((_DCIF2XX_PWMC)(((_CPWMC) this)->DerivedClass))->DVars_str

#ifdef MC_CLASS_DYNAMIC
#include "stdlib.h" /* Used for dynamic allocation */
#else
_DCIF2XX_PWMC_t IF2XX_PWMCpool[MAX_DRV_PWMC_NUM];
unsigned char IF2XX_PWMC_Allocated = 0u;
#endif
...
static volatile _DCIF2XX_PWMC oIF2XXdbg;
#endif

static void IF2XX_Init(CPWMC this);
static void IF2XX_TIMxInit(TIM_TypeDef* TIMx, CPWMC this);
static void IF2XX_CurrentReadingCalibration(CPWMC this);
static void IF2XX_GetPhaseCurrents(CPWMC this,Curr_Components* pStator_Currents);
static void IF2XX_TurnOnLowSides(CPWMC this);
static void IF2XX_SwitchOnPWM(CPWMC this);
static void IF2XX_SwitchOffPWM(CPWMC this);
static uint16_t IF2XX_WriteTIMRegisters(CPWMC this);   
static void *IF2XX_IRQHandler(void *this, unsigned char flag);
static uint16_t IF2XX_ExecRegularConv(CPWMC this, uint8_t bChannel);
static void IF2XX_ADC_SetSamplingTime(CPWMC this, ADConv_t ADConv_struct);
static void IF2XX_HFCurrentsCalibration(CPWMC this,Curr_Components* pStator_Currents);
static uint16_t IF2XX_IsOverCurrentOccurred(CPWMC this);
static uint16_t F2XX_GPIOPin2Source(uint16_t GPIO_Pin);
static void IF2XX_ADC_Deinit(ADC_TypeDef* Local_ADC);
                                    ...
                                    pICS_DDParams_t pICS_DDParams)
{
  ...
  _DCIF2XX_PWMC _oICS_F2XX;
  
  _oPWMnCurrFdbk = (_CPWMC)PWMC_NewObject(pPWMnCurrFdbkParams);
  
#ifdef MC_CLASS_DYNAMIC
  _oICS_F2XX = (_DCIF2XX_PWMC)calloc(1u,sizeof(_DCIF2XX_PWMC_t));
#else
  if (IF2XX_PWMC_Allocated  < MAX_DRV_PWMC_NUM)
  {
    _oICS_F2XX = &IF2XX_PWMCpool[IF2XX_PWMC_Allocated++];
  }
  else
  {
    _oICS_F2XX = MC_NULL;
  ...
  _oICS_F2XX->pDParams_str = pICS_DDParams;
  _oPWMnCurrFdbk->DerivedClass = (void*)_oICS_F2XX;
  
  _oPWMnCurrFdbk->Methods_str.pIRQ_Handler = &IF2XX_IRQHandler;
  
  Set_IRQ_Handler(pICS_DDParams->IRQnb, (_CMCIRQ)_oPWMnCurrFdbk);
  
  _oPWMnCurrFdbk->Methods_str.pPWMC_Init = &IF2XX_Init;
  _oPWMnCurrFdbk->Methods_str.pPWMC_GetPhaseCurrents = &IF2XX_GetPhaseCurrents;
  _oPWMnCurrFdbk->Methods_str.pPWMC_SwitchOffPWM = &IF2XX_SwitchOffPWM;
  _oPWMnCurrFdbk->Methods_str.pPWMC_SwitchOnPWM = &IF2XX_SwitchOnPWM;        
  _oPWMnCurrFdbk->Methods_str.pPWMC_CurrentReadingCalibr = 
                                                 &IF2XX_CurrentReadingCalibration;         
  _oPWMnCurrFdbk->Methods_str.pPWMC_TurnOnLowSides = &IF2XX_TurnOnLowSides;         
  _oPWMnCurrFdbk->Methods_str.pPWMC_SetADCSampPointSect1 = 
                                                      &IF2XX_WriteTIMRegisters;        
  ...
  _oPWMnCurrFdbk->Methods_str.pPWMC_SetADCSampPointSect6 = 
                                                      &IF2XX_WriteTIMRegisters; 
  _oPWMnCurrFdbk->Methods_str.pPWMC_ExecRegularConv= &IF2XX_ExecRegularConv;
  
  _oPWMnCurrFdbk->Methods_str.pPWMC_SetSamplingTime= &IF2XX_ADC_SetSamplingTime;
  
  _oPWMnCurrFdbk->Methods_str.pPWMC_IsOverCurrentOccurred = 
    &IF2XX_IsOverCurrentOccurred;
  
  return ((CIF2XX_PWMC)_oPWMnCurrFdbk);
}
...
/** @defgroup ICS_F2XX_class_private_methods ICS_F2XX class private methods
* @{
*/
...
* @param  this: related object of class CIF2XX_PWMC
* @retval none
*/
static void IF2XX_Init(CPWMC this)
{
  ...
  IF2XX_TIMxInit(pDParams_str->TIMx, this);
  ...
  IF2XX_ADC_Deinit(ADC1);
  IF2XX_ADC_Deinit(ADC2);
...
static void IF2XX_CurrentReadingCalibration(CPWMC this)
{
  ...
  ((_CPWMC) this)->Methods_str.pPWMC_GetPhaseCurrents = &IF2XX_HFCurrentsCalibration;
  
  IF2XX_SwitchOnPWM(this);
  ...
  ((_CPWMC) this)->Methods_str.pPWMC_GetPhaseCurrents = &IF2XX_GetPhaseCurrents;
}
