/**
  ******************************************************************************
  * @file    R1_HD2_PWMnCurrFdbkClass.c
  * @author  STMicroelectronics - System Lab - MC Team
  * @version 4.2.0
  * @date    20-Aug-2015 18:06
  * @brief   This file contains private implementation of R1_HD2_PWMnCurrFdbk IRQ      
  ******************************************************************************
  * @attention
  *
...
#include "R1_HD2_PWMnCurrFdbkPrivate.h"
...
#define DCLASS_VARS  ((_DCR1HD2_PWMC)(((_CPWMC) this)->DerivedClass))->DVars_str
...
#else
	_DCR1HD2_PWMC_t R1HD2_PWMCpool[MAX_DRV_PWMC_NUM];
	unsigned char R1HD2_PWMC_Allocated = 0u;
#endif

static void* R1HD2_IRQHandler(void *this, unsigned char flag);
static void R1HD2_Init(CPWMC this);
static void R1HD2_TIMxInit(TIM_TypeDef* TIMx, TIM_TypeDef* TIMx_2, CPWMC this);
static void R1HD2_CurrentReadingCalibration(CPWMC this);
static void R1HD2_GetPhaseCurrents(CPWMC this,Curr_Components* pStator_Currents);
static void R1HD2_TurnOnLowSides(CPWMC this);
static void R1HD2_SwitchOnPWM(CPWMC this);
static void R1HD2_SwitchOffPWM(CPWMC this);
static void R1HD2_1ShuntMotorVarsInit(CPWMC this);
static void R1HD2_1ShuntMotorVarsRestart(CPWMC this);
static uint16_t R1HD2_CalcDutyCycles(CPWMC this);
static uint16_t R1HD2_ExecRegularConv(CPWMC this, uint8_t bChannel);
static void R1HD2_ADC_SetSamplingTime(CPWMC this, ADConv_t ADConv_struct);
static void R1HD2_HFCurrentsCalibration(CPWMC this,Curr_Components* pStator_Currents);
static uint16_t R1HD2_IsOverCurrentOccurred(CPWMC this);
...
/**
  ...
  */
CR1HD2_PWMC R1HD2_NewObject(pPWMnCurrFdbkParams_t pPWMnCurrFdbkParams, pR1_DDParams_t pR1_DDParams)
{
	_CPWMC _oPWMnCurrFdbk;
	_DCR1HD2_PWMC _oR1_HD2;

	_oPWMnCurrFdbk = (_CPWMC)PWMC_NewObject(pPWMnCurrFdbkParams);

	#ifdef MC_CLASS_DYNAMIC
		_oR1_HD2 = (_DCR1HD2_PWMC)calloc(1u,sizeof(_DCR1HD2_PWMC_t));
	#else
		if (R1HD2_PWMC_Allocated  < MAX_DRV_PWMC_NUM)
		{
			_oR1_HD2 = &R1HD2_PWMCpool[R1HD2_PWMC_Allocated++];
		}
		else
		{
			_oR1_HD2 = MC_NULL;
	...
	_oR1_HD2->pDParams_str = pR1_DDParams;
	_oPWMnCurrFdbk->DerivedClass = (void*)_oR1_HD2;
	
	_oPWMnCurrFdbk->Methods_str.pIRQ_Handler = &R1HD2_IRQHandler;
	Set_IRQ_Handler(pR1_DDParams->IRQnb, (_CMCIRQ)_oPWMnCurrFdbk);
  
  _oPWMnCurrFdbk->Methods_str.pPWMC_Init = &R1HD2_Init;
  _oPWMnCurrFdbk->Methods_str.pPWMC_GetPhaseCurrents = &R1HD2_GetPhaseCurrents;
  _oPWMnCurrFdbk->Methods_str.pPWMC_SwitchOffPWM = &R1HD2_SwitchOffPWM;
  _oPWMnCurrFdbk->Methods_str.pPWMC_SwitchOnPWM = &R1HD2_SwitchOnPWM;        
  _oPWMnCurrFdbk->Methods_str.pPWMC_CurrentReadingCalibr = 
                                                 &R1HD2_CurrentReadingCalibration;         
  _oPWMnCurrFdbk->Methods_str.pPWMC_TurnOnLowSides = &R1HD2_TurnOnLowSides;         
  _oPWMnCurrFdbk->Methods_str.pPWMC_SetADCSampPointSect1 = 
                                                      &R1HD2_CalcDutyCycles;        
  ...
  _oPWMnCurrFdbk->Methods_str.pPWMC_SetADCSampPointSect6 = 
                                                      &R1HD2_CalcDutyCycles;
  _oPWMnCurrFdbk->Methods_str.pPWMC_ExecRegularConv= &R1HD2_ExecRegularConv;
  _oPWMnCurrFdbk->Methods_str.pPWMC_SetSamplingTime= &R1HD2_ADC_SetSamplingTime;
  _oPWMnCurrFdbk->Methods_str.pPWMC_IsOverCurrentOccurred = 
    &R1HD2_IsOverCurrentOccurred;

	return ((CR1HD2_PWMC)_oPWMnCurrFdbk);
}
...
/** @defgroup R1_HD2_class_private_methods R1_HD2 class private methods
* @{
*/
...
static void R1HD2_Init(CPWMC this)
{
  ...
  pDVars_str =   &(((_DCR1HD2_PWMC)(((_CPWMC) this)->DerivedClass))->DVars_str);
  pDParams_str =  ((_DCR1HD2_PWMC)(((_CPWMC) this)->DerivedClass))->pDParams_str;
  
  pVars_str->bMotor = (pDParams_str->bInstanceNbr==1u?M1:M2);
  
  R1HD2_1ShuntMotorVarsInit(this);
  ...
  if (pDParams_str->TIMx == TIM1)
  {
    ...
    R1HD2_TIMxInit(TIMx, TIMx_2, this);
    ...
    {
      /* Only if REP RATE > 1 */
      /* Enable the DMA1_CH5 TC interrupt */
  ...
  else
  {
    ...
    if (pDParams_str->bRepetitionCounter > 1u)
    {
      ...
      NVIC_InitStructure.NVIC_IRQChannel = (uint8_t) DMA2_Channel2_IRQn;
      NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = DMAx_TC_PRE_EMPTION_PRIORITY;
      NVIC_InitStructure.NVIC_IRQChannelSubPriority = DMAx_TC_SUB_PRIORITY;
      NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
      NVIC_Init(&NVIC_InitStructure);
  ...
  R1HD2_1ShuntMotorVarsRestart(this);
  
  /*  Set TIMx_2 CCx start value */
  if (TIMx_2 == TIM4)
  {
    TIMx_2->CCR3 = (PWM_PERIOD >> 1) - pDParams_str->hTbefore;
    TIM_DMACmd(TIMx_2, TIM_DMA_CC3, ENABLE);
  }
  if (TIMx_2 == TIM5)
  {
    TIMx_2->CCR4 = (PWM_PERIOD >> 1) - pDParams_str->hTbefore;
  ...
  ADC_ExternalTrigInjectedConvCmd(ADCx,DISABLE);
...
static void R1HD2_TIMxInit(TIM_TypeDef* TIMx, TIM_TypeDef* TIMx_2, CPWMC this)
{
  ...
  if (TIMx_2 == TIM4)
  {
    TIM_SelectOutputTrigger(TIMx_2, TIM_TRGOSource_OC3Ref);
  ...
  if (pDParams_str->bFreqRatio == 2u) 
  {
    ...
    TIM_SetCounter(TIMx, PWM_PERIOD-1u);
    TIM_SetCounter(TIMx_2, PWM_PERIOD-1u);         
  }
  else /* bFreqRatio equal to 1 or 3 */
  {
    if (pDParams_str->bInstanceNbr == 1u)
    {
      TIM_SetCounter(pDParams_str ->TIMx, PWM_PERIOD-1u);
      TIM_SetCounter(pDVars_str->TIMx_2, PWM_PERIOD-1u);
    }
  }
}
...
static void R1HD2_CurrentReadingCalibration(CPWMC this)
{
  ...
  ((_CPWMC) this)->Methods_str.pPWMC_GetPhaseCurrents = &R1HD2_HFCurrentsCalibration;
  
  R1HD2_SwitchOnPWM(this);
  ...
  ((_CPWMC) this)->Methods_str.pPWMC_GetPhaseCurrents = &R1HD2_GetPhaseCurrents;  
}
...
static void R1HD2_SwitchOnPWM(CPWMC this)
{
  ...
  /* Set all duty to 50% */
  pDParams_str->TIMx->CCR1 = PWM_PERIOD >> 1;
  pDParams_str->TIMx->CCR2 = PWM_PERIOD >> 1;
  pDParams_str->TIMx->CCR3 = PWM_PERIOD >> 1;
...
/**
  * @brief  R1_HD2 implement MC IRQ function TIMER Update and DMA TC
  * @param  this related object
  ...
  case 1: /* TIM8 Update IRQ */
    {      
      ...
      {                      
        ...
        {
        case INVERT_A:
          DMA2_Channel2->CPAR = TIM8_CCR1_Address;
          /* Stdlib replaced: TIM_DMACmd(pDVars_str->TIMx, TIM_DMA_CC4, ENABLE);*/
          pDParams_str->TIMx->DIER |= TIM_DMA_CC4;          
          break;
          
        case INVERT_B:
          DMA2_Channel2->CPAR = TIM8_CCR2_Address;
          /* Stdlib replaced: TIM_DMACmd(pDVars_str->TIMx, TIM_DMA_CC4, ENABLE);*/
          pDParams_str->TIMx->DIER |= TIM_DMA_CC4;
          break;
          
        case INVERT_C:
          DMA2_Channel2->CPAR = TIM8_CCR3_Address;
          /* Stdlib replaced: TIM_DMACmd(pDVars_str->TIMx, TIM_DMA_CC4, ENABLE);*/
          pDParams_str->TIMx->DIER |= TIM_DMA_CC4;
          break;
...
static uint16_t R1HD2_IsOverCurrentOccurred(CPWMC this)
{
  TIM_TypeDef*  LocalTIMx = ((_DCR1HD2_PWMC)(((_CPWMC) this)->DerivedClass))->
    pDParams_str->TIMx;
