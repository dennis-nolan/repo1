...
extern unsigned short speedangle;
extern unsigned short oldspeedangle;
extern signed short myspeed;
extern unsigned short start_dcnq;
...
bool STC_ExecRamp(CSTC this, int16_t hTargetFinal, uint32_t hDurationms)
{
  ...
  if (pVars->bMode == STC_TORQUE_MODE)
  {
    ...
    if ((int32_t)hTargetFinal > (int32_t)pParams->hMaxPositiveTorque)
    {
      AllowedRange = FALSE;
    }
    if (hTargetFinal < pParams->hMinNegativeTorque)
    {
      AllowedRange = FALSE;
    }
    #endif
  ...
  {
    hCurrentReference = SPD_GetAvrgMecSpeed01Hz(pVars->oSPD);
    pVars->wSpeedRef01HzExt = (int32_t)hCurrentReference * (int32_t)65536;
    #ifdef CHECK_BOUNDARY
...
/**
  ...
  *         Current(digit) = [Current(Amp) * 65536 * Rshunt * Aop]  /  Vdd micro
  */
int16_t hMeasuredSpeedReported;
...
// dcnq17 calculate new speed estimate as first order difference in angle
myspeed = speedangle - oldspeedangle;
oldspeedangle = speedangle;



//BETH 12/1/2017 DAC->DHR12L1 =  myangle;   //JDO

//DAC->DHR12L1 = 32767 + (myspeed>>4);
//DAC->SWTRIGR=1;



    /* Compute speed error */
// dcnq17  re-calibrate reference for new speed feedback
   hTargetSpeed = (int16_t)(wCurrentReference / 9972); // JDO fb is 6.67 times bigger
    hMeasuredSpeed = SPD_GetAvrgMecSpeed01Hz(pVars->oSPD);

// dcnq17  use new speed feedback to calculate error
//    hError = hTargetSpeed - hMeasuredSpeed;
    hError = hTargetSpeed - myspeed;
...
// dcnq17 output scaled speed to DAC
// DAC->DHR12L1 =  myspeed * 10;  JDO REMOVED TO CHANGE DAC
// DAC->SWTRIGR=1;
  ...
  hMeasuredSpeedReported = myspeed;
