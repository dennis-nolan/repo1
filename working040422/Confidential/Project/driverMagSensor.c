...
uint16_t valueRead; 
uint16_t prevValueRead = 0; 
#define GOING_UP        0
#define GOING_DOWN      1
uint8_t direction = GOING_UP;
uint8_t newDirection = GOING_UP;
  //--------------------------------
  // Initial position in free-space
  //--------------------------------
#define TIMER_QUEUE_SIZE 10 
uint32_t timerQueue[TIMER_QUEUE_SIZE]; 
uint8_t timerQueueOffset = 0; 
...
//        accel_packet = ((address & ACCEL_ADDR_MASK) | ACCEL_READ) << 8;
        ...
        SPI_I2S_SendData(SPI_FLASH, 0);  //accel_packet);
        ...
        if (valueRead<prevValueRead)
        {
            if (direction == GOING_UP)
            {
                //------------SAVE VALUE
                //save time
              if (timerQueueOffset >= TIMER_QUEUE_SIZE)
              {
                timerQueueOffset = 0; 
              }
              timerQueue[timerQueueOffset++] = freeRunningTimer; 
              freeRunningTimer = 0;              
            }
...
//    }  
  ...
  // then reads it back. 
 // MagChangeRegister(0x07,0x160);  
  
  
//  MagChangeRegister(0x02,214);// BCT  160 is typical on the pic steering JDO
  
  MagChangeRegister(0x02,214);// BCT  was 214
  
  
//  MagChangeRegister(0x03,0x02);// ETY =  02 = edge of magnet ring position…. ETX =  01  JDO
  MagChangeRegister(0x03,0x01);// ETY =  02 = edge of magnet ring position…. ETX =  01  JDO
  

//  MagChangeRegister(0x09,0x00);  // dcnq
