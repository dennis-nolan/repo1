...
#include "DACSPIAD7303DUAL_UserInterfacePrivate.h"
	...
	_DCDACX2_UI_t DACX_UIpool[MAX_DACX_UI_NUM];
...
#define DAC_SPI_CE2_GPIO_RCC     RCC_AHBPeriph_GPIOA
#define DAC_SPI_CE2_GPIO_PORT    GPIOA
#define DAC_SPI_CE2_GPIO_PIN     GPIO_Pin_9
#define DAC_SPI_CE2_DISABLE      Bit_SET
#define DAC_SPI_CE2_ENABLE       Bit_RESET 
...
CDACX_UI DACX_NewObject(pUserInterfaceParams_t pUserInterfaceParams, pDACSPIAD7303Params_t pDACSPIAD7303Params)
{
	...
	_DCDACX2_UI _oDACSPIAD7303;

	_oUserInterface = (_CUI)UI_NewObject(pUserInterfaceParams);

	#ifdef MC_CLASS_DYNAMIC
		_oDACSPIAD7303 = (_DCDACX2_UI)calloc(1u,sizeof(_DCDACX2_UI_t));
	#else
...
{  
  ...
  /* DAC enable 2*/
  GPIO_InitStructure.GPIO_Pin = DAC_SPI_CE2_GPIO_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(DAC_SPI_CE2_GPIO_PORT, &GPIO_InitStructure);
...
static void DACX_Exec(CUI this)
{
  uint8_t out1,out2,out3,out4;
	MC_Protocol_REG_t bCh1_var,bCh2_var,bCh3_var,bCh4_var;
  bCh1_var = ((_DCDACX2_UI)(((_CUI)this)->DerivedClass))->
    DVars_str.bChannel_variable[DAC_CH0];
  bCh2_var = ((_DCDACX2_UI)(((_CUI)this)->DerivedClass))->
    DVars_str.bChannel_variable[DAC_CH1];
  bCh3_var = ((_DCDACX2_UI)(((_CUI)this)->DerivedClass))->
    DVars_str.bChannel_variable[DAC_CH2];
  ...
  /***** Send the second two channels *****/
  
  /* Wait for SPI Tx buffer empty */ 
  while(SPI_I2S_GetFlagStatus(DAC_USE_SPI, SPI_I2S_FLAG_BSY)==SET);	
  
  /* Enable Slave */
  GPIO_WriteBit(DAC_SPI_CE2_GPIO_PORT, DAC_SPI_CE2_GPIO_PIN, DAC_SPI_CE2_ENABLE);
  /* Send SPI Control */ 
  SPI_SendData8(DAC_USE_SPI, 0x20);
  while (SPI_I2S_GetFlagStatus(DAC_USE_SPI, SPI_I2S_FLAG_BSY)==SET);
  /* Send SPI data */ 
  SPI_SendData8(DAC_USE_SPI, (u8)(out3));
  /* Wait for SPI Tx buffer empty */
  while (SPI_I2S_GetFlagStatus(DAC_USE_SPI, SPI_I2S_FLAG_BSY)==SET);
  /* Disable Slave */
  GPIO_WriteBit(DAC_SPI_CE2_GPIO_PORT, DAC_SPI_CE2_GPIO_PIN, DAC_SPI_CE2_DISABLE);
  ...
  while (SPI_I2S_GetFlagStatus(DAC_USE_SPI, SPI_I2S_FLAG_BSY)==SET);
  /* Send SPI data */ 
  SPI_SendData8(DAC_USE_SPI, (u8)(out4));
  /* Wait for SPI Tx buffer empty */
  while (SPI_I2S_GetFlagStatus(DAC_USE_SPI, SPI_I2S_FLAG_BSY)==SET);
  /* Disable Slave */
  GPIO_WriteBit(DAC_SPI_CE2_GPIO_PORT, DAC_SPI_CE2_GPIO_PIN, DAC_SPI_CE2_DISABLE);
...
static void DACX_SetChannelConfig(CUI this, DAC_Channel_t bChannel, 
                              MC_Protocol_REG_t bVariable)
{
  ((_DCDACX2_UI)(((_CUI)this)->DerivedClass))->
    DVars_str.bChannel_variable[bChannel] = bVariable;
}

static MC_Protocol_REG_t DACX_GetChannelConfig(CUI this, DAC_Channel_t bChannel)
{
  return ((_DCDACX2_UI)(((_CUI)this)->DerivedClass))->
