...
#define LCDMANAGER_VERS "LCD manager ver: 4.2"
...
void LCDI_UpdateMeasured(CUI oUI)
{
  ...
  if (pGUI_PFCStatus)
  {
    State_t PFC_Status = (State_t)(*pUI_GetReg)(oUI,MC_PROTOCOL_REG_PFC_STATUS);
    switch (PFC_Status)
    {
    case IDLE:
      {
  ...
  {
    /* Check fault */
    uint32_t PFC_Faults = (*pUI_GetReg)(oUI,MC_PROTOCOL_REG_PFC_FAULTS);
  ...
  if (pGUI_PFCEnable)
  {
    bool PFC_Enabled = (bool)(*pUI_GetReg)(oUI,MC_PROTOCOL_REG_PFC_ENABLED);
    if (PFC_Enabled == true)
    {
...
{    
  ...
  if (g_oDAC)
  {
    MC_Protocol_REG_t DAC_Channel_1_selected = (MC_Protocol_REG_t)(*pUI_GetReg)(g_oDAC,MC_PROTOCOL_REG_DAC_OUT1);
    MC_Protocol_REG_t DAC_Channel_2_selected = (MC_Protocol_REG_t)(*pUI_GetReg)(g_oDAC,MC_PROTOCOL_REG_DAC_OUT2);
    DAC_Config_t locDac_Config = LCDI_GetDACConfig(g_oUI);
    uint8_t i = 0;
...
void LCDI_Polling()
{
  ...
  if ((STM_EVAL_PBGetState(BUTTON_KEY) == BUTTON_KEY_ACTIVE) &&
...
void OnGUI_CtrlMode_ChangeText(HANDLE_WIN hWin, uint32_t wParam)
{
  ...
  if (NumericDataU8[MotorSel][GUI_CtrlMode] == GUI_TORQUE_MODE)
  {
    if ((*pUI_GetReg)(g_oUI,MC_PROTOCOL_REG_STATUS) == IDLE)
    {
      (*pUI_ExecCmd)(g_oUI,MC_PROTOCOL_CMD_IQDREF_CLEAR);
      NumericData[MotorSel][GUI_Iq_Ref] = (int16_t)(*pUI_GetReg)(g_oUI,MC_PROTOCOL_REG_I_Q_REF);
      NumericData[MotorSel][GUI_Id_Ref] = (int16_t)(*pUI_GetReg)(g_oUI,MC_PROTOCOL_REG_I_D_REF);
    }
    Page4.SetVisualizable(false);
...
void LCDI_ShowPage8(void)
{
  ...
  if ((*pUI_GetSelectedMCConfig)(g_oUI) & UI_CFGOPT_PFC_I_KD)
  {    
...
void OnGUI_PFCEnable(HANDLE_WIN hWin, uint32_t wParam)
{
  ...
  {
    /* PFC Disable */
    (*pUI_ExecCmd)(g_oUI,MC_PROTOCOL_CMD_PFC_DISABLE);
  }
  else
  {
    /* PFC Enable */
    (*pUI_ExecCmd)(g_oUI,MC_PROTOCOL_CMD_PFC_ENABLE);
  }
}

void OnGUI_PFCFaultAck_Click(HANDLE_WIN hWin, uint32_t wParam)
{
  (*pUI_ExecCmd)(g_oUI,MC_PROTOCOL_CMD_PFC_FAULT_ACK);
}
#endif
